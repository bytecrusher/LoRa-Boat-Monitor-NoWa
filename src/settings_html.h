...
{ 
  ...
  for (int i = 0; i < num; i++)
  {
    ...
    if (vname[i] == "bmeSensor") {
      //actconf.bmeSensor = toInteger(value[i]);
      actconf.bmeSensor = toInteger(value[i]);
    }
 ...
 if(actconf.crypt == 1 && (hash.length() == 0 || hash != cryptPassword(String(actconf.password)))){
  ...
  // content += CSS();
   ...
   transID();
    ...
    content += F("document.SetForm.bmeSensor.selectedIndex = ");
    content += getindex(bmeSensor, String(actconf.bmeSensor));
    content += F(";");
 ...
 */
    ...
    content += F("<tr>");
    content += F("<td>BME Sensor Type</td>");
    content += F("<td>");
    content += F("<select name='bmeSensor' size='1'>");
    content += F("<option value='0'>Off</option>");
    content += F("<option value='1'>On</option>");
    content += F("</select>");
    content += F("</td>");
