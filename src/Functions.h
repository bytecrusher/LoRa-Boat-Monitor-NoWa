...
String cryptPassword(String password){
  ...
  md5.add(raw);
  md5.calculate();
  mycrypt = md5.toString();         // GH: renamed
  ...
  DebugPrintln(3, mycrypt);         // GH: renamed
  // Give back the crypted password
  return mycrypt;                   // GH: renamed
}
...
void readValues() {
    ...
    DebugPrint(3, "bmeSensor = ");
    // Digital relay output high activ 
    DebugPrint(3, actconf.bmeSensor);
    DebugPrintln(3, "  ");
    ...
    if((inByte==13) && (start==1)) { // Detect end of NMEA0183 telegram (CR)
      ...
      if (nmea_all.substring(3,6) == "RMC") {
        ...
        if(CheckNMEA(nmea)) {
          ...
          if (actconf.bmeSensor == 1) {
            DebugPrint(3, "BME280 ");
            DebugPrintln(3, String(actconf.bmeSensor));
            DebugPrintln(3, "");
          }
